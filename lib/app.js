...
/*
 * I'm pretty sure this should be removed. We have a /api/favorites
 * route, and this query wouldn't work as intended.
 * 
 */
/*
// GET Favorites
...
app.get('/api/recommendations', async(req, res) => {
  try {
    ...
    const breeds = await getBreedsByNames(breedNames, user.state, user.zipcode);
    res.json(breeds);
  } catch(e) {
    res.status(500);
  }
...
app.get('/api/breed-details/:breed_name', async(req, res)=> {
  try{
    ...
    const breed = await getBreedByName(req.params.breed_name, userProfile.state, userProfile.zipcode);
    res.json(breed); 
  }catch(e){
    res.status(500);
  }
